...
async function getWeing(req, res) {
    try {
        ...
        // fetch the posts
        let kost = await db.collection("kost").find({}).sort({}).toArray();
        // return the posts
        return res.json({
            message: JSON.parse(JSON.stringify(kost)),
            success: true
        });
    } catch (error) {
        // return the error
...
async function deleteWeing(req, res) {
    ...
    try {
        ...
        let { db  } = await connectToDatabase();
        // Deleting the post
        await db.collection("kost").deleteOne({
...
async function addWeding(req, res) {
    try {
        ...
        let { db  } = await connectToDatabase();
        // add the post
        await db.collection("kost").insertOne(JSON.parse(req.body));
        // return a message
        return res.json({
...
var __webpack_exports__ = __webpack_require__.X(0, [198], () => (__webpack_exec__(8722)));
